...
class SkeletonGuideSpine(OpenMayaUI.MPxLocatorNode):
    id = typeid.GUIDESPINE
    drawDbClassification = "drawdb/geometry/skeletonGuideSpine"
    drawRegistrantId = "SkeletonGuideSpinePlugin"
    ...
    def initialize(cls):
        ...
        cls.guideMatrix = matrixAttr.create("guideMatrix", "gm", OpenMaya.MFnMatrixAttribute.kDouble)
        cls.addAttribute(cls.guideMatrix)

        cls.handleInverseScale = nAttr.create("handleInverseScale", "his", OpenMaya.MFnNumericData.k3Double, 1.0)
        ...
        # cls.attributeAffects(cls.handleMatrix, cls.handlePosition)
        # cls.attributeAffects(cls.guideMatrix, cls.handlePosition)
    ...
    def __init__(self):
        super(SkeletonGuideSpine, self).__init__()

    def postConstructor(self):
        nodeFn = OpenMaya.MFnDependencyNode(self.thisMObject())
        nodeFn.setName("skeletonGuideSpineShape#")
        plug = nodeFn.findPlug("forgeID", False)
        plug.setString(util.generateID())

    def compute(self, plug, dataBlock):
        boundsRelatedPlugs = self.boundingBoxCorner1, self.boundingBoxCorner2, self.aimVector, self.upVector
    ...
    def computeBounds(self, plug, dataBlock):
        ...
        guidePosition = OpenMaya.MVector(OpenMaya.MTransformationMatrix(guideMatrix).translation(OpenMaya.MSpace.kPostTransform))
        ...
        # handles = dataBlock.inputArrayValue(self.handleMatrix)
        ...
        #     handlePosition = OpenMaya.MVector(OpenMaya.MTransformationMatrix(inMatrix).translation(OpenMaya.MSpace.kPostTransform))
        ...
        # outputArray = dataBlock.outputArrayValue(self.handlePosition)
        # builder = OpenMaya.MArrayDataBuilder(dataBlock, self.handlePosition, len(projectedPoints))
        ...
        # dataBlock.setClean(self.handlePosition)
    ...
    def computeInverseScale(self, plug, dataBlock):
        ...
        scale = OpenMaya.MTransformationMatrix(invScaleMatrix).scale(OpenMaya.MSpace.kPostTransform)
...
class GuideSpineData(OpenMaya.MUserData):
    def __init__(self):
        super(GuideSpineData, self).__init__(False) # deleteAfterUse=False
        self.boundingBoxCorner1 = None
        self.boundingBoxCorner2 = None
        self.outputPoints = None

class SkeletonGuideSpineDrawOverride(OpenMayaRender.MPxDrawOverride):

    GuideClass = SkeletonGuideSpine

    def __init__(self, obj):
        super(SkeletonGuideSpineDrawOverride, self).__init__(obj, SkeletonGuideSpineDrawOverride.draw)

    @staticmethod
    def creator(obj):
        return SkeletonGuideSpineDrawOverride(obj)
